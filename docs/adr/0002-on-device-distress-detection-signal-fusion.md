...
- `KeywordDetector`: TensorFlow Lite / DSP PCM analysis for configured trigger words ("help", "save me", "stop") with confidence scores and model version logging.
- `ScreamDetector`: Hybrid audio feature pipeline (audio windowing, zero-crossing rate, peak amplitude ratio) coupled with TensorFlow Lite YAMNet classifier inference (`yamnet.tflite`, version 1.0) evaluating scream/yell/shout event classes ("Screaming", "Yell", "Shout").
